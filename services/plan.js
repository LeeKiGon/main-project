...
const findAllPublicPlans = async ({ page, user, destination, style }) => {
    ...
    if(style === undefined) {
        ...
        const findPage = await Plan.find({destination : {$in : destination}, status:'공개'}).sort('-createdAt').skip(5 * (page - 1)).limit(5).populate('userId likeCount bookmarkCount', 'snsId email nickname profile_img')

        const plansLikeBookmark = await Plan.findLikeBookmark(findPage, user);
        return { plans: plansLikeBookmark, endPage }
    } else {
        const numPlans = await Plan.count({destination: {$in : destination}, style : {$all : style}, status : '공개'})
        const endPage = numPlans === 0 ? 1 : Math.ceil(numPlans / 5)
        const findByStyle = await Plan.find({destination : {$in : destination}, style : {$all : style}, status : '공개'}).sort('-createdAt').skip(5 * (page - 1)).limit(5).populate('userId likeCount bookmarkCount', 'snsId email nickname profile_img')
